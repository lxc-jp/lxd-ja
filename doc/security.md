...
コンテナ間のデータ共有が必要ない場合は、`security.idmap.isolated`（{doc}`instance-options`参照）を有効にすることで、各コンテナに対して重複しないUID/GIDマップを使用し、他のコンテナに対する潜在的な{abbr}`DoS（サービス拒否）`攻撃を防ぐことができます。
