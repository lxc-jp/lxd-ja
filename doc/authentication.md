...
我々はクライアントとサーバーの両方を管理しているので、壊れたプロトコルやサーバーの下位互換をサポートする理由はありません。
壊れたプロトコルや暗号の後方互換性をサポートする理由はありません。
...
1. サーバーはクライアントの認証を試みます。
   - クライアントの証明書がサーバーのトラストストアにある場合は、接続が許可されます。
...
クライアントへの信頼を取り消すには、`lxc config trust remove FINGERPRINT`でそのクライアントの証明書をサーバーから削除します。
...
信頼できるクライアントを追加するには、そのクライアントの証明書をサーバーのトラストストアに直接追加するのが望ましい方法です。
これを行うには、クライアントの証明書をサーバーにコピーし、`lxc config trust add <file>`で登録します。
...
{abbr}`PKI (Public key infrastructure)`の設定では、システム管理者が中央のPKIを管理し、すべてのlxcクライアント用のクライアント証明書とすべてのLXDデーモン用のサーバー証明書を発行します。
...
このモードでは、LXDデーモンへの接続はすべて、あらかじめ用意されたCA証明書を使って行われます。
事前に発行されたCA証明書を使用します。
 ...
 * 接続が傍受されている（{abbr}`MITM (Man in the middle)`）。
...
信頼されている他のクライアントや、ローカルのサーバー管理者が、サーバー上のクライア ントの信頼エントリを削除すると、そのクライアントに対するサーバーの信頼関係は失効します。
