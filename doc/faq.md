...
リッスンするアドレスを設定するには、利用可能なアドレスを調べた上でサーバで `config set` コマンドを実行します。
...
LXD コンテナはカーネルモジュールをロードすることはできないので、お使いの Docker の設定によっては、ホストで追加のカーネルモジュールをロードする必要があることに注意してください。
