...
多くのネットワークにまたがって複数のインスタンスからなる LXD クラスタを運用する際に有用です。
...
オペレータのプロダクションの DNS サーバへのゾーン転送のためのこれらのゾーンを
サーブします。
...
- 下流ネットワークポートに対して (下流 OVN ネットワークを持つアップリンクのネットワーク上に設定されれうネットワークゾーンに対して): `<project_name>-<downstream_network_name>.uplink.lxd.example.net`
...
全てのインスタンス、ネットワークゲートウェイ、下流ネットワークポートに対して
ネットワークゾーンは正引きと逆引きレコードを自動的に生成します。
