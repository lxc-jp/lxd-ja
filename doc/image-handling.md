...
## 特別なイメージプロパティ
プレフィックス***requirements***で始まるイメージプロパティ(例：requirements.XYZ)
は、LXDがホストシステムと当該イメージで生成されるインスタンスの互換性を判断するために使用されます。
インスタンスの互換性を判断するために使用されます。これらの互換性がない場合には
LXDはそのインスタンスを起動しません。
...
requirements.cgroup     | string | -          | "v1" "に設定されている場合、イメージはホストがCGroupV1を実行することを必要とすることを示します。
